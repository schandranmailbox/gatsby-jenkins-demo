...
const introspectAndStoreRemoteSchema = async () => {
  ...
  if (staleIntrospectionData && printSchemaDiff) {
    ...
    staleIntrospectionData.__schema.types.forEach(type => {
      ...
      const typeDiff = (0, _lodash.uniqBy)(diff.diffJson(type, newType), `value`);

      if (typeDiff.length) {
        console.log(`\nFound changes to the ${type.name} type\n`);
        typeDiff.forEach(part => {
          if (part.added) {
            console.log(_chalk.default.green(_chalk.default.bold(`Added:\n`) + part.value.trim().split(`\n`).map((line, index) => `+${index === 0 ? `\t` : ` `}${line}`).join(`\n`)));
          } else if (part.removed) {
            console.log(_chalk.default.red(_chalk.default.bold(`Removed:\n`) + part.value.trim().split(`\n`).map((line, index) => `-${index === 0 ? `\t` : ` `}${line}`).join(`\n`)));
          }
        });
        console.log(`\n`);
      }
